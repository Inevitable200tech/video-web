...
import { Eye, Calendar, User, Share2, ThumbsUp, Tag, Play, Heart, Lock } from "lucide-react";
...
export default function WatchVideo() {
  ...
  const checkLoginWall = () => {
    ...
    if (el.duration && (el.currentTime / el.duration) >= 0.5) {
      el.pause();
      // Drop the video source completely to prevent DOM manipulation bypass
  ...
  useEffect(() => {
    if (!playbackData?.playbackUrl || !videoRef.current || showLoginWall) return;
    const el = videoRef.current;
    el.src = playbackData.playbackUrl;
    el.load();
  }, [playbackData?.playbackUrl, showLoginWall]);
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-black border border-white/5 shadow-2xl">
              {isLoading ? (
                ...
                <div className="relative aspect-video">
                  ...
                  {showLoginWall && !user && (
                    <div 
                      className="absolute inset-0 flex flex-col items-center justify-center bg-black z-30 overflow-hidden"
                      style={{
                    ...
                    >
                      ...
                      <div className="relative z-10 flex flex-col items-center px-4 text-center">
                        <div className="w-16 h-16 bg-primary/20 rounded-full flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(59,130,246,0.3)]">
                          <Lock className="w-8 h-8 text-primary" />
                        </div>
                        <h3 className="text-2xl sm:text-3xl font-bold text-white mb-3 tracking-tight">Sign Up to Continue Watching</h3>
                        <p className="text-white/70 mb-8 max-w-md text-sm sm:text-base font-medium">
                          You've reached the 50% preview limit for guests. Create a free account to watch the full video and access our entire premium library!
                        </p>
                        <Link href="/login">
                          <Button size="lg" className="font-bold tracking-widest uppercase rounded-full px-8 h-14 bg-primary text-primary-foreground shadow-[0_0_40px_rgba(59,130,246,0.4)] hover:shadow-[0_0_60px_rgba(59,130,246,0.6)] hover:scale-105 transition-all">
                            Create Free Account
                          </Button>
                        </Link>
                      </div>
          ...
          <div className="space-y-6">
            {isLoading ? (
              ...
              const isAutoDiscovered = video?.description === "Auto-discovered from storage network." || !video?.description;
              const uploaderName = isAutoDiscovered ? "Anonymous" : "Network Contributor";
              const uploaderRole = isAutoDiscovered ? "User" : "Verified Uploader";
              const videoCategory = video?.category === "Cinema" ? "Desi Vids" : (video?.category || "Desi Vids");
              const videoDescription = isAutoDiscovered ? "Video By anonymous" : video?.description || "No description provided.";
              ...
              <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-6 py-4 border-y border-white/5">
                  <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-foreground">{uploaderName}</p>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{uploaderRole}</p>
                      </div>
                    </div>
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
                  <div className="flex items-center gap-2 text-primary text-[10px] font-bold uppercase tracking-widest mb-4">
                    ...
                    {videoCategory} • {video?.uploadedAt && formatDistanceToNow(new Date(video.uploadedAt))} ago
                  </div>
